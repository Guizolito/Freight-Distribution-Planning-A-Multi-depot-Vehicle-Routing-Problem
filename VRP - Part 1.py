...
def ScenarioSolver(ScenarioFile,ReturnOrN): #Entrance variable corresponds to the respective scenario file
    ...
    # Keeps track of the Total Costs, Time and Distance of each Truck
    ...
    model.objective = minimize(xsum(arcBin[i][j][k] * DistanceMatrix[i][j] * TruckDistCost[k] for k in NTrucks for j in V2 for i in V) + xsum(UsageMatrix[k] * TruckRentCost[k] for k in NTrucks))
    ...
    print("Waiting Time: " + str(TotalTime.x) + " --> " + str(TimeConversion(TotalTime.x)))
    ...
    plt.suptitle("Scenario 2 - MDVRP", fontsize = 20, fontname="Times New Roman", fontweight = "bold")
    ...
    # Adds the arrows for a clearer view of the route

...
def ScenarioSolver(ScenarioFile, ReturnOrN): #Entrance variable corresponds to the respective scenario file
    ...
    TotalCO2 = model.add_var()

    TotalTime = model.add_var()
    ...
    TotalTime = xsum(TTime[i][k] for k in NTrucks for i in V1)

    for k in NTrucks:

        TotalDistance[k] = xsum(arcBin[i][j][k] * DistanceMatrix[i][j] for j in V2 for i in V)

    TotalCO2 = xsum(DistanceMatrix[i][j] * arcBin[i][j][k] * CO2[k] for i in V for j in V2 for k in NTrucks)

    # Objective function - Minimizes delivery time and service time

    model.objective = minimize(TotalCosts + TotalTime)
    ...
    print("Total Time: " + str(TotalTime.x) + " --> " + str(TimeConversion(TotalTime.x)))
    ...
    plt.suptitle("Scenario 3 - Multi-Objective MDVRP", fontsize = 20, fontname="Times New Roman", fontweight = "bold")
    ...
    print("CO2 emissions: " + str(TotalCO2.x))
...
StartTime = time.time()
ScenarioSolver("C:\\Users\\gui_t\\Desktop\\Assignment 1\\Scenarios\\Scenario3.txt",ReturnOrNot)
EndTime = time.time()
